...
uint256 hashGenesisBlock("0xbbd01e417e370842498a91038fd40da95f4a6166a231a3bed3c4671207bd8296"); //doubloons
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Doubloons: starting difficulty is 1 / 2^12
...
{
    // Doubloons: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 6.77 * COIN; //doubloons
...
{
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact(); //doubloons
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Doubloons: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
    {
        ...
        {
            return state.DoS(10, error("ProcessBlock() : block with too little proof-of-work"));
        }
    }
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        pchMessageStart[0] = 0xfb; //doubloons
...
unsigned char pchMessageStart[4] = { 0xfc, 0xd9, 0xb7, 0xdd }; // Doubloons
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DoubloonsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DoubloonsMiner(CWallet *pwallet)
{
    printf("DoubloonsMiner started\n");
    ...
    try { loop {
        ...
        printf("Running DoubloonsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DoubloonsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DoubloonsMiner, pwallet));
}
